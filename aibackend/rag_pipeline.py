...
GRADE_PROMPT = (
    ...
    "Return a JSON object with exactly one field 'binary_score' set to either 'yes' or 'no' to indicate whether the document is relevant to the question."
)
...
def grade_documents_node(state: RAGState) -> RAGState:
    ...
    response = grader.with_structured_output(GradeDocuments, method="json_mode").invoke(
...
def rewrite_question_node(state: RAGState) -> RAGState:
    ...
    if router:
        ...
        try:
            decision = router.with_structured_output(RewriteStrategy, method="json_mode").invoke(
                [{"role": "user", "content": prompt}]
            )
